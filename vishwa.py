...
st.title(':red[Youtube Data Scraping And Harvesting]')
...
st.write('''TO get data and to store it in  MongoDB database, click below **:blue['Get data and store']**.''')
Get_data = st.button(":green[**Get data and store**]")

#youtube api
youtube=build("youtube",'v3',developerKey="AIzaSyDUM-1s9oY5SiW0MxC2u09dnxi-lJGrwt0")
...
# create a database or use existing one
    mydb = client['Youtube_DB2']

# create a collection
    collection = mydb['Youtube_data2']
...
# insert or update data in the collection
    ...
    st.write("**:blue[Loaded successfully]**") 
    client.close()
#Data Migrate Zone
...
mycursor.execute("CREATE DATABASE IF NOT EXISTS youtube_db2")

        # Close the cursor and database connection
mycursor.close()
connect.close()
Check_channel = st.checkbox('**:red[Check available channel data for analysis]**')

if Check_channel:
   # Create database connection
    engine = create_engine('mysql+pymysql://root:98655@localhost/youtube_db2', echo=False)
    ...
    try:
        ...
        for i in df_at_sql['Available channel data']:
            if i==document_name:
                st.write("**:red [CHANNEL DETAILS ALREADY EXISTS IN MYSQL,SO MOVE TO CHANNEL ANALYSIS]**")
                break
    except:
            st.write('**:blue[THERE IS ONLY ONE CHANNEL]**.')

st.write(''' To Migrate to MySQL database from MongoDB database, click below **:blue['Migrate to MySQL']**.''')
...
#     # playlist data json to df
    ...
    st.write(":blue[Migrated Successfully]")
...
connect_for_question = pymysql.connect(host='localhost', user='root', password='98655', db='youtube_db2')
